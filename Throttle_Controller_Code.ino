/*
  ...
  Revision History:
  1.2.1 - Added scaling based on measured voltages for the potentiometers
  ...
  Add code to adjust the min and max values based on what is seen because heat affects the resistance and therefore the measured throttle.
*/
//INCLUDES//
#include <Wire.h>
...
void loop() {
  if (digitalRead(Direction_Pin) == 1) { //If the controller is in the normal position
    Throttle_1_Value = map(analogRead(Throttle_1_Pin), T1_Min*1023/Vcc, T1_Max*1023/Vcc, 0, 100); //Map input 1 to 0-100% given the min and max voltages measured
    Throttle_2_Value = map(analogRead(Throttle_2_Pin), T2_Min*1023/Vcc, T2_Max*1023/Vcc, 0, 100); //Map input 2 to 0-100% given the min and max voltages measured
